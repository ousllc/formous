...
フォーム送信時または次へボタン押下時にエラーがある場合、グローバルエラー表示領域にエラーメッセージが表示されます。
グローバルエラーはリアルタイムバリデーション時に更新されず、フォーム送信時または次へボタン押下時にのみその内容が表示されます。
```html
<!-- フォーム下部にまとめて表示 -->
<div class="global-errors" data-validation-error-global>
  <div data-validation="error" data-validation-type="required" 
...
また、以下のHTML標準属性を使用すると、自動的にバリデーションが適用されます：
```html
<input 
  required                  <!-- 必須入力 -->
  minlength="3"            <!-- 最小文字数 -->
